 ...
 * 三处界面:settings.section「Ollama 用量」完整面板、sidebar.footer.action
 * 极简双横条、tool.view.cordis Run 卡片同款面板。数据经 /ollama-usage 前缀
 * 路由走 Connection RPC 信封协议(check / snapshot / forget)。
 */
  ...
  factory: (require) => {
    ...
    function apply(ctx) {
      ...
      const rpc = (endpoint, payload) => {
        if (connection == null || typeof connection.rpc !== 'object' || typeof connection.rpc.call !== 'function') {
        ...
        React.useEffect(() => {
          ...
          const refresh = () => rpc('snapshot', {}).then((res) => {
            ...
            if (res && res.ok && res.value && res.value.usage) {
              ...
              // snapshot 读自磁盘(插件无内存缓存),必然已持久化;服务端字段优先,缺省按不变式置 true
              setPersisted(typeof res.value.persisted === 'boolean' ? res.value.persisted : true)
            }
          }).catch(() => {})
          refresh()
